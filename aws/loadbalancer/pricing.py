...
import sys
import json
from utils.utils import get_region_name, get_region_code, get_price1
...
class Pricing:
    ...
    def get_lb_price(self, product_family):    # Returns loadbalancer price
        price = 0
        try:
            ...
            f = lb_filter.format(r=self.formatted_region, pf=product_family, ut=usageType)
            ...
            if error.response['Error']['Code'] == 'LimitExceededException':
                ...
                self.logger.error(error.response['Error']['Code'] + ': ' + error.response['Error']['Message'] +
                                  ' | Line {} in loadbalancer pricing.py'.format(sys.exc_info()[-1].tb_lineno))
                sys.exit(1)
