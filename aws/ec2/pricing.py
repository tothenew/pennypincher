...
class Pricing:
    ...
    def get_os(self, os):  # Returns OS of the AMI used in EC2
        try:
    ...
    def get_preinstalled_sw(self, os):  # Returns pre installed software information in AMI used in EC2
        try:
    ...
    def get_license(self, os):  # Returns license information of EC2 AMI
        try:
    ...
    def get_ec2_price(self, instance_type, operating_system):  # Returns the price of EC2
        try:
            os = self.get_os(operating_system)
            lic = self.get_license(operating_system)
            software = self.get_preinstalled_sw(operating_system)
            f = ec2_filter.format(r=self.formatted_region, t=instance_type, o=os, l=lic, s=software)
            ...
            if error.response['Error']['Code'] == 'LimitExceededException':
                ...
                self.logger.error(error.response['Error']['Code'] + ': ' + error.response['Error']['Message'] +
                                  ' | Line {} in ec2 pricing.py'.format(sys.exc_info()[-1].tb_lineno))
                sys.exit(1)
        except Exception as e:
