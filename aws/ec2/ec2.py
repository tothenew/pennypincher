...
class EC2:
    ...
    def _get_image_info(self, client, image_id):  # Returns AMI information
        response = client.describe_images(ImageIds=[image_id])
        return response['Images']

    def _get_savings(self, ec2_list):  # Returns total possible savings
        savings = 0
        for ec2 in ec2_list:
    ...
    def _get_findings(self, state, avg_cpu, max_cpu, net_in_out, status):
        ...
        if status.lower() == 'adhoc':
            ...
            if state.lower() == 'stopped':
                ...
                if avg_cpu < 1 and max_cpu < 1 and net_in_out < 5000000:  # EC2 idle condition check
                    finding = 'Idle'
        return finding

    def _describe_ec2(self, client):  # Returns information of all existing ec2's  in json format
        all_instances = client.describe_instances()
        return all_instances['Reservations']

    def _get_ebs_size(self, client, volume_id):  # Returns size of the EBS attached
        ebs = client.describe_volumes(VolumeIds=[volume_id], )
        size = ebs['Volumes'][0]['Size']
        return float(size)

    def get_result(self):  # Returns a list of lists which contains headings and idle EC2 information
        try:
            ec2_list = []
                    ...
                    for instance in r['Instances']:
                        if 'SpotInstanceRequestId' not in instance:
                            ...
                            if finding == 'Idle':
                                ...
                                ec2.append(instance_name)
                                ec2.append(instance['InstanceType'])
                                ec2.append(vpc_id)
                                ec2.append(str(instance['LaunchTime'].date()))
                                ec2.append(reg)
                                ec2.append(finding)
                                ec2.append(round(savings * 732, 2))
                                ec2_list.append(ec2)
            # To fetch top 10 resources with maximum saving
            ec2_sorted_list = sorted(ec2_list, key=lambda x: x[7], reverse=True)
            total_savings = self._get_savings(ec2_sorted_list)
            return ec2_sorted_list, headers, total_savings
