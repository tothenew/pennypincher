...
import sys
import json
from utils.utils import get_region_name, get_price1, get_price
...
class Pricing:
    ...
    def get_es_price(self, instance_type, storage_type, volume_size, iops_count):  # Returns elasticsearch price
        iops_price = 0
        try:
            storage_type = self._get_es_storage(storage_type)
            f = elasticsearch_filter.format(r=self.formatted_region, i=instance_type, )
            ...
            f = elasticsearch_storage_filter.format(r=self.formatted_region, s=storage_type)
            ...
            if error.response['Error']['Code'] == 'LimitExceededException':
                ...
                self.logger.error(error.response['Error']['Code'] + ': ' + error.response['Error']['Message'] +
                                  ' | Line {} in elasticsearch pricing.py'.format(sys.exc_info()[-1].tb_lineno))
                sys.exit(1)
