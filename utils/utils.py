...
def convert_size(size_in_bytes):   # Returns size in a human readable format
    if size_in_bytes == 0:
        return "0B"
...
def get_region_name(region_code):  # Returns region name corresponding to the region abbreviation
    try:
...
def get_region_list():   # Returns a list of valid regions for an AWS account
    try:
        regions = []
...
def get_backup_age(creation_date):   # Returns age of the backup
    today = datetime.datetime.now(timezone.utc).date()
    delta = today - creation_date
    return delta.days


def get_region_code(region_code):   # Returns region code corresponding to an AWS region abbreviation
    try:
...
def get_price(data):   # Fetches and returns price from pricing api json
...
def get_price1(data):   # Fetches and returns price from pricing api json
    od = json.loads(data['PriceList'][0])['terms']['OnDemand']
    id1 = list(od)[0]
    id2 = list(od[id1]['priceDimensions'])[0]
    price = float(od[id1]['priceDimensions'][id2]['pricePerUnit']['USD'])
...
def get_price2(data):  # Fetches and returns price from pricing api json
